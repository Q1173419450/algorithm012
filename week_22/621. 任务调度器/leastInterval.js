...
var leastInterval = function(tasks, n) {
  ...
  for(let i = 0; i < tasks.length; i++) {
    ...
    let index = tasks[i].charCodeAt(0) - codePointA; 
    counts[index]++;
  }

  // 对数组进行排序，优先排列个数（count）最大的任务，
  ...
  while (i >= 0 && counts[i] == maxCount) {
    retCount++; // 一个任务B个数和最大任务数一致，则在最后加上B任务即可
    i--;
  }
